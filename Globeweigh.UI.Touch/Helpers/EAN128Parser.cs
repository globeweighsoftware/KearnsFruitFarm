...
namespace Globeweigh.UI.Touch.Helpers
{
    public static class EAN128Parser
    {
        ...
        static EAN128Parser()
        {
            ...
            Add("21", "SerialNumber", 2, DataType.Numeric, 20, true);
        ...
        public static int GetSerialNumber(string barcodeData)
        {
            HasCheckSum = false;
            EAN128StartCode = "1";
            Dictionary<AII, string> result = Parse(barcodeData);

            var serialNumber = result.FirstOrDefault(a => a.Key.AI == "21").Value;
            if (serialNumber != null) return Convert.ToInt32(serialNumber);
            return 0;
        }
